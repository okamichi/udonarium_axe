...
- **2D 多方向閲覧** — 卓を四方から囲んで読めるよう、キャラクター名とバフ名を一文字ずつ円周に沿わせて時計回りに表示。短いラベルは4個、長さに応じて3～1個へ減らし、`◆` と余白で区切る。コマ画像と台座は別の速度で平面回転し、HP・MPバーや状態アイコンは固定する。連続回転と90°ごとの間欠回転、各一周時間と静止時間をテーブル設定で変更できる（[利用方法](multi-angle.md)）
...
- **外周ティッカー** — 固定の「ティッカー」タブへ投稿した公開発言を、2Dモードの画面四辺へ時計回りに流す。ON/OFFと速度をタブ内で変更でき、投稿待ちでは最後の内容を繰り返す。短文は `◆` 区切りで最大8個まで複製して外周へ等間隔に置き、長文では重ならない個数まで自動的に減らす（[利用方法](multi-angle.md#外周ティッカー)）
