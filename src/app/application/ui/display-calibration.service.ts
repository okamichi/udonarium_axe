...
export class DisplayCalibrationService {
  ...
  private readonly _distanceValue = signal<number>(DEFAULT_CELL_DISTANCE_VALUE);
  private readonly _distanceUnit = signal<CellDistanceUnit>(DEFAULT_CELL_DISTANCE_UNIT);
  ...
  /**
   * What one square is worth in the rules. Read here rather than shared, since nobody else's
   * view changes because of what this table counts by.
   */
  readonly distanceValue = this._distanceValue.asReadonly();
  readonly distanceUnit = this._distanceUnit.asReadonly();
  ...
  setDistance(value: number, unit: CellDistanceUnit): void {
    this._distanceValue.set(asCellDistanceValue(value));
    this._distanceUnit.set(asCellDistanceUnit(unit));
  }
  ...
  reset(): void {
    ...
    this._distanceValue.set(DEFAULT_CELL_DISTANCE_VALUE);
    this._distanceUnit.set(DEFAULT_CELL_DISTANCE_UNIT);
