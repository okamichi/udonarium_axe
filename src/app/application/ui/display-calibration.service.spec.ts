...
describe('DisplayCalibrationService', () => {
  ...
  describe('nothing is written down', () => {
    it('starts every session unmeasured, since no screen can be identified', () => {
      ...
      first.setDistance(1.5, 'm');

      const second = service();

      expect(second.isCalibrated()).toBe(false);
      expect(second.realSizeEnabled()).toBe(false);
      expect(second.distanceValue()).toBe(5);
      expect(second.distanceUnit()).toBe('ft');
    });
    ...
    it('leaves no trace in the browser', () => {
      ...
      calibration.setDistance(1.5, 'm');

      expect(localStorage.getItem('ui-display-calibration')).toBeNull();
    });
  });

  describe('what one square is worth in the rules', () => {
    it('starts at the five feet the usual rules count by', () => {
      const calibration = service();

      expect(calibration.distanceValue()).toBe(5);
      expect(calibration.distanceUnit()).toBe('ft');
    });

    it('is settable before the screen has ever been measured, being a rule and not a measurement', () => {
      const calibration = service();

      calibration.setDistance(1.5, 'm');

      expect(calibration.distanceValue()).toBe(1.5);
      expect(calibration.distanceUnit()).toBe('m');
      expect(calibration.isCalibrated()).toBe(false);
    });

    it('survives a measurement taken after it', () => {
      const calibration = service();
      calibration.setDistance(1.5, 'm');
    ...
    it('falls back rather than keeping something unusable', () => {
      const calibration = service();

      calibration.setDistance(-1, 'leagues' as never);
  ...
  describe('when the screen changes under a measurement', () => {
    ...
    it('keeps the warning standing while unrelated settings are changed', () => {
      ...
      // Neither of these says anything about the screen, so neither may clear the warning.
      calibration.setDistance(1.5, 'm');
      expect(calibration.needsRecalibration()).toBe(true);
